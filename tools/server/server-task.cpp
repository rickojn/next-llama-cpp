...
server_prompt * server_prompt_cache::alloc(const server_prompt & prompt, size_t state_size) {
    ...
    for (auto it = states.begin(); it != states.end(); ++it) {
        const int cur_lcp_len = it->tokens.get_common_prefix_ignore_thinking(prompt.tokens).cache_prefix_len;
    ...
    for (auto it = states.begin(); it != states.end();) {
        const int len = it->tokens.get_common_prefix_ignore_thinking(prompt.tokens).cache_prefix_len;
...
bool server_prompt_cache::load(server_prompt & prompt, const server_tokens & tokens_new, llama_context * ctx, int32_t id_slot) {
    const int lcp_best = prompt.tokens.get_common_prefix_ignore_thinking(tokens_new).cache_prefix_len;
    ...
    for (auto it = states.begin(); it != states.end(); ++it) {
        const int lcp_cur = it->tokens.get_common_prefix_ignore_thinking(tokens_new).cache_prefix_len;
